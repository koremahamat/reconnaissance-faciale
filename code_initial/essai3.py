...
def sauvegarder_image(frame, box, nom_reconnu):
    """Sauvegarde l'image capturée dans le répertoire spécifié avec date et heure."""
    x1, y1, x2, y2 = box.astype(int)
    face_image = frame[y1:y2, x1:x2]
    
    # Récupérer la date et l'heure actuelle
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    face_path = os.path.join(dataset_dir, f"face_{timestamp}.jpg")

    # Convertir en image PIL pour ajouter du texte
    pil_image = Image.fromarray(face_image)
    draw = ImageDraw.Draw(pil_image)
    font = ImageFont.load_default()
    
    # Ajouter le texte avec le nom reconnu
    draw.text((10, 10), nom_reconnu, fill="white", font=font)

    pil_image.save(face_path)
    logging.info(f"Sauvegarde de l'image à : {face_path}")

def dessiner_cadran(frame, box):
    """Dessine un cadran autour du visage détecté."""
    x1, y1, x2, y2 = box.astype(int)
        ...
        for box in boxes:
            ...
            if face_tensor is not None and face_tensor.size(0) > 0:
                ...
                if distances[best_match_index] < 0.6:  # Seuil de reconnaissance
                    ...
                    sauvegarder_image(frame, box, nom_reconnu)  # Sauvegarder l'image
                    dessiner_cadran(frame, box)  # Dessiner le cadran autour du visage
